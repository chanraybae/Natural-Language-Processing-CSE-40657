...
def getline(prompt=''):
    ...
    while True:
        ...
        # Provide the last 4 characters as context, edited for ngram
        context = chars[-4:] if len(chars) >= 4 else ['<BOS>'] * (4 - len(chars)) + chars
        for i in range(20):
            next_char_probs = {char: lm.get_probability(char, tuple(context)) for char in lm.vocab}
            c = max(next_char_probs, key=next_char_probs.get)
            if c == '<EOS>':
                break
            prediction.append(c)
            context = (context + [c])[-4:]
...
if __name__ == "__main__":
    ...
    # ngram task 2 lines replace
    lm = ngram.NgramModel(5)
    data = [list(line.rstrip('\n')) for line in open(args.train)]
    lm.train(data)
